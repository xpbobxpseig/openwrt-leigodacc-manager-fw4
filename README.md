...
- ✅ **tproxy_ip 守护** — cron 每 5 分钟检测并自动修复 daemon 硬编码的伪造 IP，不中断运行中的加速
- ✅ **离家模式** — 异地加速时禁止自动暂停；LuCI 一键开关 + AJAX 实时生效 + 状态表刷新
- ✅ **daemon 自愈** — 检测 acc-gw 崩溃循环 (stop_acc) 自动强重启，打破死循环
- ✅ **自定义空闲间隔** — 1~30 分钟可调检测频率
- ✅ **兼容 fw3/fw4** — 自动检测防火墙类型，动态选择依赖包
- ✅ **内核网络调优** — BBR 拥塞控制 + fq_codel + conntrack_max=65536
