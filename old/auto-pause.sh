...
# --------------- idle detection ---------------
check_all_idle() {
    # Check acc_core_conf.json for active acceleration (daemon's real-time state)
    # Pattern: "state":1 means actively accelerating
    if [ -f /tmp/acc/acc_core_conf.json ]; then
        if grep -q '"state":1' /tmp/acc/acc_core_conf.json 2>/dev/null; then
            return 1  # found active acceleration → NOT idle
        fi
    fi

    # Fallback: check UCI state
...
check_gameacc_active() {
    # If GAMEACC chain has rules beyond the header, game is running
    local count
